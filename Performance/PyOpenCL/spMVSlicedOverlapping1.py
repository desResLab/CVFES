...
if __name__ == "__main__":
    ...
    pinnedM = cl.Buffer(context, mem_flags.READ_WRITE | mem_flags.ALLOC_HOST_PTR, MAX_NSmp_Bytes)
    pinnedV = cl.Buffer(context, mem_flags.READ_WRITE | mem_flags.ALLOC_HOST_PTR, v.shape[0]*MAX_NSmp*8)
    pinnedRes = cl.Buffer(context, mem_flags.READ_WRITE | mem_flags.ALLOC_HOST_PTR, y.shape[0]*MAX_NSmp*8)
    ...
    srcM, _eventSrcM = cl.enqueue_map_buffer(queues[0], pinnedM, map_flags.WRITE, 0, (M.shape[0], MAX_NSmp, 3, 3), M.dtype)
    srcV, _eventSrcV = cl.enqueue_map_buffer(queues[0], pinnedV, map_flags.WRITE, 0, (v.shape[0], MAX_NSmp), v.dtype)
    srcRes, _eventSrcRes = cl.enqueue_map_buffer(queues[0], pinnedRes, map_flags.READ, 0, (y.shape[0], MAX_NSmp), y.dtype)
    ...
    for i in range(LOOP_COUNT):
        ...
        for iSliced in range(nslicedSmp):
            ...
            eventV0 = cl.enqueue_copy(queues[0], vector_buf, srcV)
            ...
            program.matrix_dot_vector(queues[0], (globalWorkSize,), (localWorkSize,),
                                      np.int64(halfSize), np.int64(0), np.int64(MAX_NSmp), np.int64(nlocalSmp),
