''' In order to deal with 'ndarray is not continuous' when sliced is needed in device,
    transform the M into (nSmp, m, 3, 3), may cause latency in computation due to coerleased data loading.
'''
...
if __name__ == "__main__":
    ...
    M = np.transpose(M, (1, 0, 2, 3)).copy(order='C')
    v = np.ones((nSmp, m*3))
    y = np.zeros((nSmp, m*3))
    ...
    MAX_NSmp = int(MAX_NBytes / (M.shape[1] * 72))
    MAX_NSmp_Bytes = MAX_NSmp * M.shape[1] * 72
    ...
    vector_buf = cl.Buffer(context, mem_flags.READ_ONLY, v.shape[1]*MAX_NSmp*8)
    destination_buf = cl.Buffer(context, mem_flags.WRITE_ONLY, y.shape[1]*MAX_NSmp*8)
    ...
    kernelsource = open("spMVSlicedOverlapping3.cl").read()
