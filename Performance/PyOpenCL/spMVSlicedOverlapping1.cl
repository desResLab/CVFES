...
__kernel void matrix_dot_vector(
        long N, long offset,
        const long maxNSmp, const long nSmp,
        ...
        __global double *result)
{
    for (uint y = get_group_id(0)+offset; y < N; y += get_num_groups(0))
    {
        ...
        // Row pointer
        const __global double* row = M + row_s * 9 * maxNSmp;
        ...
        for (uint s = get_local_id(0); s < nSmp; s += get_local_size(0))
        {
            ...
            for (uint x = 0; x < row_e - row_s; ++x)
            {
                ...
                sum_a += row[x*9*maxNSmp+s*9]*V[j*3*maxNSmp+s] + \
                         row[x*9*maxNSmp+s*9+1]*V[(j*3+1)*maxNSmp+s] + \
                         row[x*9*maxNSmp+s*9+2]*V[(j*3+2)*maxNSmp+s];
                sum_b += row[x*9*maxNSmp+s*9+3]*V[j*3*maxNSmp+s] + \
                         row[x*9*maxNSmp+s*9+4]*V[(j*3+1)*maxNSmp+s] + \
                         row[x*9*maxNSmp+s*9+5]*V[(j*3+2)*maxNSmp+s];
                sum_c += row[x*9*maxNSmp+s*9+6]*V[j*3*maxNSmp+s] + \
                         row[x*9*maxNSmp+s*9+7]*V[(j*3+1)*maxNSmp+s] + \
                         row[x*9*maxNSmp+s*9+8]*V[(j*3+2)*maxNSmp+s];
            }

            result[y*3*maxNSmp+s] = sum_a;
            result[(y*3+1)*maxNSmp+s] = sum_b;
            result[(y*3+2)*maxNSmp+s] = sum_c;
