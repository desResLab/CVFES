...
__kernel void calc_u(const long nSmp, const long ndof, const double dt,
                       ...
                       __global double *ures)
{
    for (uint i = get_group_id(0); i < ndof; i += get_num_groups(0))
    {
        for (uint j = get_local_id(0); j < nSmp; j += get_local_size(0))
        {
            ures[i*nSmp+j] = (dt*dt*(P[i*nSmp+j] - Ku[i*nSmp+j]) + LM[i*nSmp+j]*(2.0*u[i*nSmp+j] - up[i*nSmp+j])) / LHS[i*nSmp+j];
        }
    }
}
