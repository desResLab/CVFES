...
cdef void assembling(long[::1] eNIds, double[::1] lLHS, double[:,::1] lRHS,
                     ...
                     double[::1] Res, double[:,::1] lMResT1, double[:,::1] lMResT2,
                     double[:,::1] lMResT3, double[:,::1] lMResT4, double[:,::1] lMResT5,
                     double[:,::1] lMResT6, double[:,::1] lMResT7, double[::1] lPResT1, double[::1] lPResT2,
                     double[::1] mRT1, double[::1] mRT2, double[::1] mRT3, double[::1] mRT4,
                     double[::1] mRT5, double[::1] mRT6, double[::1] mRT7, double[::1] pRT1,
                     double[::1] pRT2):
...
#                      double[::1] Res):
    ...
    for a in range(nPts):
        ...
        for b in range(3):
            ...
            mRT6[eNIds[a]*3+b] += lMResT6[a,b]
            mRT7[eNIds[a]*3+b] += lMResT7[a,b]
...
def OptimizedExplicitVMSAssemble(
    ...
    double[:,:,::1] sdu, double[:,:,::1] nsdu, double[:,::1] sp, double[:,::1] nsp,
    double[:,::1] f, double[::1] hs,
    double[:,::1] lN, double[:,::1] lDN, double[::1] w, double[::1] coefs,
    double[::1] LHS, double[::1] RHS, double[::1] R, double[::1] mRT1, double[::1] mRT2,
    double[::1] mRT3, double[::1] mRT4, double[::1] mRT5, double[::1] mRT6, double[::1] mRT7,
...
#     double[::1] LHS, double[::1] RHS, double[::1] R):
    ...
    cdef double tmpM, varT1, varT2, varT3
    cdef double ph, hph, sph, sduhNorm
    ...
    cdef double[::1] lNsp = np.zeros(nPts, dtype=np.float)
    # for debugging only
    ...
    cdef double[:,::1] lMomentumResT6 = np.empty((nPts, 3), dtype=np.float)
    cdef double[:,::1] lMomentumResT7 = np.empty((nPts, 3), dtype=np.float)
    ...
    for iElm in range(nElms):
        ...
        for i in range(nPts):
            ...
            for j in range(4):
                ...
                lMomentumResT6[i,j] = 0.0
                lMomentumResT7[i,j] = 0.0
        ...
        # For pressure subscale
        tau_p = (h**2) * tau_u_inv
        tau_t_p = 1.0 / (1.0/dt + invEpsilon/tau_p)

        # Evaluate velocity sub-grid scales
        for i in range(nPts):
            nsp[iElm,i] = sp[iElm,i]*tau_t_p/dt - tau_t_p*invEpsilon*trGradU
            for j in range(ndim):
        ...
        for iGp in range(nGp):
            ...
            for i in range(nPts):
                ...
                lNsp[i] += tau_t_p*invEpsilon*trGradU*lN[iGp,i]*wGp
            ...
            sph = sp[iElm,0]*lN[iGp,0] + sp[iElm,1]*lN[iGp,1] \
                + sp[iElm,2]*lN[iGp,2] + sp[iElm,3]*lN[iGp,3]
            # ah dot GradHu
            ...
            # # GradHu dot sdu
            # gradHuSduh[0] = gradHu[0,0]*sduh[0] + gradHu[0,1]*sduh[1] + gradHu[0,2]*sduh[2]
            # gradHuSduh[1] = gradHu[1,0]*sduh[0] + gradHu[1,1]*sduh[1] + gradHu[1,2]*sduh[2]
            # gradHuSduh[2] = gradHu[2,0]*sduh[0] + gradHu[2,1]*sduh[1] + gradHu[2,2]*sduh[2]
            # # the square of sduh
            # sduhNorm = 0.5*(sduh[0]**2 + sduh[1]**2 + sduh[2]**2)

            # lRHS
            for a in range(nPts):
                varT1 = ah[0]*DN[0,a] + ah[1]*DN[1,a] + ah[2]*DN[2,a]
                varT2 = sduh[0]*DN[0,a] + sduh[1]*DN[1,a] + sduh[2]*DN[2,a]
                # varT3 = DN[0,a] + DN[1,a] + DN[2,a]

                # T1[0] = nu*(gradHu[0,0]*DN[0,a]+gradHu[0,1]*DN[1,a]+gradHu[0,2]*DN[2,a]) \
                #         - (hph+sph-sduhNorm)*DN[0,a] - sduh[0]*varT1
                # T1[1] = nu*(gradHu[1,0]*DN[0,a]+gradHu[1,1]*DN[1,a]+gradHu[1,2]*DN[2,a]) \
                #         - (hph+sph-sduhNorm)*DN[1,a] - sduh[1]*varT1
                # T1[2] = nu*(gradHu[2,0]*DN[0,a]+gradHu[2,1]*DN[1,a]+gradHu[2,2]*DN[2,a]) \
                #         - (hph+sph-sduhNorm)*DN[2,a] - sduh[2]*varT1
                ...
                # T1[0] = nu*(gradHu[0,0]*DN[0,a]+gradHu[0,1]*DN[1,a]+gradHu[0,2]*DN[2,a]) \
                #         + gradHp[0]*lN[iGp,a] - sduh[0]*varT1
                # T1[1] = nu*(gradHu[1,0]*DN[0,a]+gradHu[1,1]*DN[1,a]+gradHu[1,2]*DN[2,a]) \
                #         + gradHp[1]*lN[iGp,a] - sduh[1]*varT1
                # T1[2] = nu*(gradHu[2,0]*DN[0,a]+gradHu[2,1]*DN[1,a]+gradHu[2,2]*DN[2,a]) \
                #         + gradHp[2]*lN[iGp,a] - sduh[2]*varT1

                # T1[0] = nu*(symGradHu[0,0]*DN[0,a]+symGradHu[0,1]*DN[1,a]+symGradHu[0,2]*DN[2,a]) \
                #         + gradHp[0]*lN[iGp,a] - sduh[0]*varT1
                # T1[1] = nu*(symGradHu[1,0]*DN[0,a]+symGradHu[1,1]*DN[1,a]+symGradHu[1,2]*DN[2,a]) \
                #         + gradHp[1]*lN[iGp,a] - sduh[1]*varT1
                # T1[2] = nu*(symGradHu[2,0]*DN[0,a]+symGradHu[2,1]*DN[1,a]+symGradHu[2,2]*DN[2,a]) \
                #         + gradHp[2]*lN[iGp,a] - sduh[2]*varT1
                ...
                # lRes[a,0] += wGp*((ahGradHu[0]-gradHuSduh[0]-fh[0])*lN[iGp,a] + T1[0])
                # lRes[a,1] += wGp*((ahGradHu[1]-gradHuSduh[1]-fh[1])*lN[iGp,a] + T1[1])
                # lRes[a,2] += wGp*((ahGradHu[2]-gradHuSduh[2]-fh[2])*lN[iGp,a] + T1[2])
                # lRes[a,3] += wGp*(trGradHu*lN[iGp,a]-varT2)*invEpsilon
                ...
                lMomentumResT2[a,0] += wGp*nu*(gradHu[0,0]*DN[0,a]+gradHu[0,1]*DN[1,a]+gradHu[0,2]*DN[2,a])
                lMomentumResT2[a,1] += wGp*nu*(gradHu[1,0]*DN[0,a]+gradHu[1,1]*DN[1,a]+gradHu[1,2]*DN[2,a])
                lMomentumResT2[a,2] += wGp*nu*(gradHu[2,0]*DN[0,a]+gradHu[2,1]*DN[1,a]+gradHu[2,2]*DN[2,a])

                # lMomentumResT2[a,0] += wGp*nu*(symGradHu[0,0]*DN[0,a]+symGradHu[0,1]*DN[1,a]+symGradHu[0,2]*DN[2,a])
                # lMomentumResT2[a,1] += wGp*nu*(symGradHu[1,0]*DN[0,a]+symGradHu[1,1]*DN[1,a]+symGradHu[1,2]*DN[2,a])
                # lMomentumResT2[a,2] += wGp*nu*(symGradHu[2,0]*DN[0,a]+symGradHu[2,1]*DN[1,a]+symGradHu[2,2]*DN[2,a])

                lMomentumResT3[a,0] -= wGp*(hph)*DN[0,a]
                lMomentumResT3[a,1] -= wGp*(hph)*DN[1,a]
                lMomentumResT3[a,2] -= wGp*(hph)*DN[2,a]

                # lMomentumResT3[a,0] -= wGp*(hph+sph)*DN[0,a]
                # lMomentumResT3[a,1] -= wGp*(hph+sph)*DN[1,a]
                # lMomentumResT3[a,2] -= wGp*(hph+sph)*DN[2,a]

                # lMomentumResT3[a,0] -= wGp*hph*varT3
                # lMomentumResT3[a,1] -= wGp*hph*varT3
                # lMomentumResT3[a,2] -= wGp*hph*varT3

                # lMomentumResT3[a,0] += wGp*gradHp[0]*lN[iGp,a]
                # lMomentumResT3[a,1] += wGp*gradHp[1]*lN[iGp,a]
                # lMomentumResT3[a,2] += wGp*gradHp[2]*lN[iGp,a]
                ...
                # lMomentumResT6[a,0] -= wGp*gradHuSduh[0]*lN[iGp,a]
                # lMomentumResT6[a,1] -= wGp*gradHuSduh[1]*lN[iGp,a]
                # lMomentumResT6[a,2] -= wGp*gradHuSduh[2]*lN[iGp,a]

                # lMomentumResT7[a,0] += wGp*sduhNorm*DN[0,a]
                # lMomentumResT7[a,1] += wGp*sduhNorm*DN[1,a]
                # lMomentumResT7[a,2] += wGp*sduhNorm*DN[2,a]
        ...
        for a in range(nPts):
            ...
            nsp[iElm,a] += lNsp[a] / lLHS[a]

        # Assembling
        # assembling(eNIds, lLHS, lRHS, lRes, LHS, RHS, R)

        # only for debugging
        assembling(eNIds, lLHS, lRHS, lRes, LHS, RHS, R, lMomentumResT1,
            lMomentumResT2, lMomentumResT3, lMomentumResT4, lMomentumResT5,
            lMomentumResT6, lMomentumResT7, lPressureResT1, lPressureResT2,
            mRT1, mRT2, mRT3, mRT4, mRT5, mRT6, mRT7, pRT1, pRT2)
...
@cython.boundscheck(False)
@cython.wraparound(False)
cdef void initialAssembling(long[::1] eNIds, double[::1] lLHS, double[:,::1] lRes,
                            double[::1] LHS, double[::1] Res):
...
@cython.boundscheck(False)
@cython.wraparound(False)
def OptimizedExplicitVMSInitialAssemble(
    double[:,::1] nodes, long[:,::1] elements,
    double[:,::1] du, double[::1] p, double[:,::1] f,
    double[:,::1] lN, double[:,::1] lDN, double[::1] w, double[::1] coefs,
    double[::1] LHS, double[::1] R):
    ...
    cdef double[:,::1] gradU = np.empty((ndim, ndim), dtype=np.float)
    cdef double[::1] fh = np.empty(ndim, dtype=np.float)
    cdef double[::1] duhGradU = np.empty(ndim, dtype=np.float)
    cdef double[::1] T1 = np.empty(ndim, dtype=np.float)
    cdef double[::1] lLHS = np.empty(nPts, dtype=np.float)
    cdef double[:,::1] lRes = np.empty((nPts, 4), dtype=np.float)
    ...
    for iElm in range(nElms):
        ...
        for iGp in range(nGp):
            ...
            # ah dot GradHu
            duhGradU[0] = duh[0]*gradU[0,0] + duh[1]*gradU[1,0] + duh[2]*gradU[2,0]
            duhGradU[1] = duh[0]*gradU[0,1] + duh[1]*gradU[1,1] + duh[2]*gradU[2,1]
            duhGradU[2] = duh[0]*gradU[0,2] + duh[1]*gradU[1,2] + duh[2]*gradU[2,2]

            # lRHS
            for a in range(nPts):

                T1[0] = nu*(gradU[0,0]*DN[0,a]+gradU[0,1]*DN[1,a]+gradU[0,2]*DN[2,a]) - ph*DN[0,a]
                T1[1] = nu*(gradU[1,0]*DN[0,a]+gradU[1,1]*DN[1,a]+gradU[1,2]*DN[2,a]) - ph*DN[1,a]
                T1[2] = nu*(gradU[2,0]*DN[0,a]+gradU[2,1]*DN[1,a]+gradU[2,2]*DN[2,a]) - ph*DN[2,a]

                lRes[a,0] += wGp*((duhGradU[0]-fh[0])*lN[iGp,a] + T1[0])
                lRes[a,1] += wGp*((duhGradU[1]-fh[1])*lN[iGp,a] + T1[1])
                lRes[a,2] += wGp*((duhGradU[2]-fh[2])*lN[iGp,a] + T1[2])
                lRes[a,3] += wGp*(trGradU*lN[iGp,a])*invEpsilon


        # Assembling
        initialAssembling(eNIds, lLHS, lRes, LHS, R)
